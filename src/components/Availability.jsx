    ...
    <section className="space-y-5">
      ...
      {best ? (
        <div className="panel p-4">
          ...
          <p className="t-num text-ac text-sm mt-1">
          ...
          <button className="btn btn-ac w-full mt-3.5" onClick={() => onFijar(best)}>
            Fijar este día
          </button>
        ...
        <div className="panel p-5 text-center">
          ...
          <p className="text-sm text-tx/55 mt-1.5">Sé el primero: toca una casilla arriba.</p>
        </div>
      )}
    </section>
  );
    ...
    <>
      ...
      {DIAS.map(d => {
        ...
        return (
          ...
          >
            <span className={`t-num text-sm font-semibold ${on ? '' : n ? 'text-ac' : 'text-tx/30'}`}>
              {n || '·'}
            </span>
          </button>
        );
